...
Vercel AI SDK UI Message Stream Protocol v1. The SSE
event format used between backend and frontend.

## Key Facts

- Backend advertises via response header:
  `x-vercel-ai-ui-message-stream: v1`
- Frontend (assistant-ui) parses events natively
- Emitted from PHP via Symfony `ServerEvent` objects
  yielded into an `EventStreamResponse`
...
- `data-drafted-fields` is a custom event type not in
  the Vercel spec -- carries the full drafted field map
- The "typing" effect comes from `text-delta` events,
  not from field-level deltas
