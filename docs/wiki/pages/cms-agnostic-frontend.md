...
The React app has zero knowledge of Drupal. It depends
only on the OpenAPI contract.

## Key Facts

- React 19 + TypeScript (strict mode)
- Runs standalone with Express mock API (`npm run dev`)
- Mock server in `app/server/` provides in-memory
  state, fake SSE, and real Mistral calls for drafting
- Cookie auth inherited from CMS -- no API keys in
  the frontend
- IIFE bundle built by Vite, loaded by Drupal from
  `app/dist/`
