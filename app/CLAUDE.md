...
messages, PR descriptions, etc.). The client should not know we
use Claude Code.

CLAUDE.md files are local development aids only. Never commit them
to the repository. They are listed in `.git/info/exclude` to
prevent accidental addition. Never override this with `git add -f`.
...
@.claude/skills/ticket-workflow.md
