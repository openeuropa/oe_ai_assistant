...
/**
 * Lists media entities allowed by a specific media reference field.
 */
#[FunctionCall(
  id: 'oe_ai_assistant:lookup_media',
  function_name: 'lookup_media',
  name: 'Lookup Media',
  description: 'Returns accessible media entities allowed by a specific media reference field on a host bundle. Use this before drafting entity reference data for media fields.',
  group: 'oe_ai_assistant',
  module_dependencies: ['media'],
  context_definitions: [
    ...
    'field_name' => new ContextDefinition(
      ...
      description: new TranslatableMarkup("The media reference field machine name on the host bundle."),
      required: TRUE,
    ),
    'query' => new ContextDefinition(
      data_type: 'string',
      label: new TranslatableMarkup("Query"),
      description: new TranslatableMarkup("Search text used to match media labels."),
      required: FALSE,
    ),
...
class LookupMedia extends FunctionCallBase implements StructuredExecutableFunctionCallInterface {
  ...
  public function execute() {
    ...
    $mediaDefinition = $this->entityTypeManager->getDefinition('media');
    $bundleKey = $mediaDefinition->getKey('bundle');
    $labelKey = $mediaDefinition->getKey('label');

    if (!is_string($bundleKey) || !is_string($labelKey)) {
      throw new \LogicException('The media entity type must define label and bundle keys.');
    }
    ...
    if ($bundleIds !== []) {
      $query->condition($bundleKey, $bundleIds, 'IN');
    }

    $mediaIds = $query->execute();
    $entities = $storage->loadMultiple($mediaIds);
